<!doctype html>
    <html lang="en">
        <head>
            <title>Employee Accountability</title>
            <style>
            ...
            <x-app-layout>
                ...
                <div class=" grid sm:grid-cols-3 relative overflow-x-auto">
                    ...
                    <div class="max-w-sm p-6 bg-white border border-gray-300 rounded-lg shadow dark:border-gray-200 dark:hover:bg-gray-200 text-center h-28 ml-5">
                        ...
                        @if(isset($empdetails) && is_countable($empdetails) && count($empdetails) > 0)
                                        ...
                                        @foreach($emp_dev_acc as $empdevacc)
                                            @if(!empty($empdetails) && $empdevacc->is_accountability == $empdetails[0]->EmployeeID)
                                                <tr class="bg-white border-b dark:border-gray-300 text-black">
                                                    ...
                                                    <td class="px-6 py-4 font-bold uppercase">{{$empdevacc->issue_date}}</td>
                                                    ...
                                                    <td class="px-6 py-4">
                                                        <div class="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-1 lg:grid-cols-1 xl:grid-cols-1 gap-1 ml-3">
                                                            <button type="button" onclick="location.href='{{ url('empaccdevice/'.$empdevacc->id) }}'">
                        ...
                        <div class="hidden" id="returned" role="tabpanel" aria-labelledby="returned-tab">
                            <div class="relative overflow-x-auto">
                                ...
                                <table id="dataTable1" class="w-full text-xs text-left rtl:text-right font-light text-surface text-black border-collapse">
                                    ...
                                    <tbody>
                                        @isset($emp_dev_acc_returned)
                                        @foreach($emp_dev_acc_returned as $empdevacc_returned)
                                            @if(!empty($empdetails) && $empdevacc_returned->is_accountability == $empdetails[0]->EmployeeID)
                                                <tr class="bg-white border-b dark:border-gray-300 text-black">
                                                    <td class="px-6 py-4">
                                                        {{$empdevacc_returned->id}}
                                                    </td>
                                                    <td class="px-6 py-4 font-bold uppercase">{{$empdevacc_returned->HostID}}</td>
                                                    <td class="px-6 py-4">{{$empdevacc_returned->Type}}</td>
                                                    <td class="px-6 py-4">{{$empdevacc_returned->Brand}}</td>
                                                    <td class="px-6 py-4">{{$empdevacc_returned->Model}}</td>
                                                    <td class="px-6 py-4">{{$empdevacc_returned->SerialNo}}</td>
                                                    <td class="px-6 py-4 font-bold uppercase">{{$empdevacc_returned->issue_date}}</td>
                                                    <td class="px-6 py-4 font-bold uppercase">{{$empdevacc_returned->return_date}}</td>
                                                    <td class="px-6 py-4">{{$empdevacc_returned->Location}}</td>
                                                </tr>
                                            @endif
                                    @endforeach
                                    @endisset
