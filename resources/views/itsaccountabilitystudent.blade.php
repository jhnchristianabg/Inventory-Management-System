<!doctype html>
    <html lang="en">
        <head>
            <title>Accountability / Students</title>
        </head>
        <body>
            <x-app-layout>
                <x-slot name="header">
                    ...
                    <div class="mb-3 bg-green-100 border-t-4 border-green-500 rounded-b text-teal-900 px-4 py-3 shadow-md" role="alert">
                        <div class="flex">
                            ...
                            <div>
                                ...
                                <p class="text-sm">Success! The Student details has been saved.</p>
                            </div>
                        </div>
                    </div>
                    {{ Session::get('success')}}
                ...
                @if(Session::get('failed'))
                    <div class="mb-3 bg-red-100 border-t-4 border-red-500 rounded-b text-teal-900 px-4 py-3 shadow-md" role="alert">
                        <div class="flex">
                            ...
                            <div>
                                <p class="font-bold">Inventory Management System</p>
                                <p class="text-sm">Failed! The student you entered is already exist.</p>
                            </div>
                        </div>
                    </div>
                    {{ Session::get('failed')}}
                ...
                @if(Session::get('update'))
                    <div class="mb-3 bg-green-100 border-t-4 border-green-500 rounded-b text-teal-900 px-4 py-3 shadow-md" role="alert">
                        <div class="flex">
                            ...
                            <div>
                                <p class="font-bold">Inventory Management System</p>
                                <p class="text-sm">Success! The student details has been updated.</p>
                            </div>
                        </div>
                    </div>
                    {{ Session::get('update')}}
                ...
                <div id="crud-modal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%)] max-h-full backdrop-blur-sm">
                    <div class="relative p-4 w-full max-w-md max-h-full" style="margin-right:250px">
                        ...
                        <div class="relative bg-white rounded-lg shadow-lg dark:bg-blueGray-100 px-4" style="width:650px">
                            ...
                            <!-- Modal body -->
                            <form class="p-4 md:p-5" action="add_student" method="POST">
                                {{ csrf_field() }}
                                ...
                                <div class="mt-10">
                                    <button type="submit" class="inline-flex text-white inline-flex items-center bg-green-700 hover:bg-green-800 focus:ring-4 focus:outline-none focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800" style="margin-left:395px">
                ...
                <div class="relative overflow-x-auto mt-14">
                    ...
                    <table id="dataTable" class="w-full text-xs text-left rtl:text-right font-light text-surface text-black ">
                        <thead class="text-xs uppercase bg-gray-200 font-bold">
                            <tr>
                                <th scope="col" class="px-6 py-3">
                                    <a href="{{ route('itsaccountabilitystudent.show', ['column' => 'id', 'direction' => ($column == 'id' && $direction == 'asc') ? 'desc' : 'asc']) }}">
                                        ID
                                        @if($column == 'id')
                                            @if($direction == 'asc')
                                ...
                                <th scope="col" class="px-6 py-3">
                                    <a href="{{ route('itsaccountabilitystudent.show', ['column' => 'StudentID', 'direction' => ($column == 'StudentID' && $direction == 'asc') ? 'desc' : 'asc']) }}">
                                        STUDENT ID
                                        @if($column == 'StudentID')
                                            @if($direction == 'asc')
                                            <svg class="w-4 h-4 inline" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                ...
                                <th scope="col" class="px-6 py-3">
                                    <a href="{{ route('itsaccountabilitystudent.show', ['column' => 'StudentFName', 'direction' => ($column == 'StudentFName' && $direction == 'asc') ? 'desc' : 'asc']) }}">
                                        STUDENT'S NAME
                                ...
                                <th scope="col" class="px-6 py-3">
                                    <a href="{{ route('itsaccountabilitystudent.show', ['column' => 'Department', 'direction' => ($column == 'Department' && $direction == 'asc') ? 'desc' : 'asc']) }}">
                                        DEPARTMENT
                                ...
                                <th scope="col" class="px-6 py-3">
                                    <a href="{{ route('itsaccountabilitystudent.show', ['column' => 'Year', 'direction' => ($column == 'Year' && $direction == 'asc') ? 'desc' : 'asc']) }}">
                                        YEAR LEVEL
                                        @if($column == 'Year')
                                            @if($direction == 'asc')
                                            <svg class="w-4 h-4 inline" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                ...
                                <th scope="col" class="px-6 py-3">
                                    <a href="{{ route('itsaccountabilitystudent.show', ['column' => 'Email', 'direction' => ($column == 'Email' && $direction == 'asc') ? 'desc' : 'asc']) }}">
                                        EMAIL
                                        @if($column == 'Email')
                                            @if($direction == 'asc')
                                            <svg class="w-4 h-4 inline" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        ...
                        <tbody>
                            @foreach($studentview as $itsaccountabilitystudent)
                            <tr class="bg-white border-b dark:border-gray-300 text-black">
                                <td class="px-6 py-4">{{$itsaccountabilitystudent->id}}</td>
                                <td class="px-6 py-4 font-bold uppercase">{{$itsaccountabilitystudent->StudentID}}</td>
                                <td class="px-6 py-4">{{$itsaccountabilitystudent->StudentFName}}<a class="ml-1">{{$itsaccountabilitystudent->StudentIName}}. <a class="ml-1">{{$itsaccountabilitystudent->StudentLName}}</td>
                                <td class="px-6 py-4">{{$itsaccountabilitystudent->Department}}</td>
                                <td class="px-6 py-4">{{$itsaccountabilitystudent->Year}}</td>
                                <td class="px-6 py-4">{{$itsaccountabilitystudent->Email}}</td>
                                <td class="px-6 py-4">
                                    <div class="flex justify-center gap-2">
                                        <button type="button" onclick="location.href='{{ url('studaccview/'.$itsaccountabilitystudent->id) }}'">
                                        ...
                                        <button type="button" onclick="location.href='{{ url('studentedit/'.$itsaccountabilitystudent->id) }}'">
                    ...
                    <div class="mt-3">
                        {{ $studentview->appends(['column' => $column, 'direction' => $direction])->links() }}
                    </div>
